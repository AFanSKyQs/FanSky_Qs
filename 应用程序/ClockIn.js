...
export class ClockIn extends plugin {
    ...
    async setCard(e,nowCardNum){
        ...
        for(let i=0;i < Group.length; i++){
            await Bot.pickGroup(Group[i]).setCard(Bot.uin, name + '今已打卡:' + nowCardNum + '人 | 新年快乐喵~' )
    ...
    async SingleTest(e) {
        ...
        if (!e.isMaster && SignDay[e.group_id] && SignDay[e.group_id][e.user_id] && (Date.now() - SignDay[e.group_id][e.user_id].time) < 120000) {
            //将本次计数写入json
    ...
    async MsgList(e, Data, LastTimeTemp, SignTop,TempRough) {
        ...
        let NowTime=new Date(Data[e.group_id][e.user_id].time).toLocaleString();
        ...
        const UserHtml = {
            ...
            CardDay: Data[e.group_id][e.user_id].day,   //打卡天数
            CardContinuous: Data[e.group_id][e.user_id].continuous,  //连续打卡天数
            CardCount: Data[e.group_id][e.user_id].count,   //总打卡次数
            CardRough: Data[e.group_id][e.user_id].rough,   //粗糙值
            CardTime: Data[e.group_id][e.user_id].time,  //打卡时间戳
    ...
    async FirstList(e, Data, SignTop,TempRough) {
        ...
        let LastTime = new Date(Data[e.group_id][e.user_id].time).toLocaleString()
        let Words =await getWords()
        const UserHtml = {
            addRough:TempRough,
            AcgBg:AcgBg,
            ...
            CardContinuous: Data[e.group_id][e.user_id].continuous,  //连续打卡天数
            CardCount: Data[e.group_id][e.user_id].count,   //总打卡次数
            CardRough: Data[e.group_id][e.user_id].rough,   //原石总计
            CardTime: Data[e.group_id][e.user_id].time,  //打卡时间戳
            CardLastTime: LastTime, //上次打卡时间
